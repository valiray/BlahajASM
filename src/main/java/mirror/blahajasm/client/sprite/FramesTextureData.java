...
import mirror.blahajasm.common.internal.mixins.TextureMapAccessor;
    ...
    public static void registerEvictionListener(ColorHandlerEvent.Block event) {
        ((IReloadableResourceManager) Minecraft.getMinecraft().getResourceManager()).registerReloadListener((ISelectiveResourceReloadListener) (manager, predicate) -> {
            if (predicate.test(VanillaResourceType.MODELS)) {
                ...
                BlahajLogger.instance.info("Evicted {} sprites' frame texture data", count);
                BlahajLogger.instance.debug("While evicting sprites' frame texture data, the following classes were skipped: [{}]", skippedSpriteClasses.stream().map(Class::getName).collect(Collectors.joining(", ")));
                canReload = true;
            }
        });
    }
