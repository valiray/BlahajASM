...
public abstract class ItemEnchantmentAmuletMixin implements IBauble {
    ...
    public void onUnequipped(ItemStack stack, EntityLivingBase player) {
        ...
        for (ItemStack armorStack : player.getArmorInventoryList()) {
            EnchantmentUpgradeHelperInvoker.blahajasm$removeAmuletOwner(armorStack);
        }
        EnchantmentUpgradeHelperInvoker.blahajasm$removeAmuletOwner(player.getHeldItemMainhand());
        EnchantmentUpgradeHelperInvoker.blahajasm$removeAmuletOwner(player.getHeldItemOffhand());
    }
