...
import mirror.blahajasm.patches.*;
...
public class BlahajTransformer implements IClassTransformer {
    ...
    public static boolean isOptifineInstalled, isSodiumPortInstalled;    public static boolean squashBakedQuads = BlahajConfig.instance.squashBakedQuads;

    Multimap<String, Function<byte[], byte[]>> transformations;

    public BlahajTransformer() {
        BlahajLogger.instance.info("BlahajASM is now preparing to bytecode manipulate your game.:3");
        isOptifineInstalled = BlahajReflector.doesClassExist("optifine.OptiFineForgeTweaker");
        isSodiumPortInstalled = BlahajReflector.doesClassExist("me.jellysquid.mods.sodium.client.SodiumMixinTweaker");
        if (squashBakedQuads) {
            if (isOptifineInstalled) {
                ...
                squashBakedQuads = false;
                BlahajLogger.instance.info("A sodium port is installed. BakedQuads won't be squashed as it is incompatible with Sodium.");
            }
            }
        ...
        if (BlahajLoadingPlugin.isClient) {
            ...
            if (squashBakedQuads) {
                ...
                addTransformation("mirror.blahajasm.bakedquad.BakedQuadFactory", BakedQuadFactoryPatch::patchCreateMethod);
    ...
    private byte[] extendSupportingBakedQuadInstead(byte[] bytes) {
        ...
        if (node.superName.equals("net/minecraft/client/renderer/block/model/BakedQuad")) {
            node.superName = "mirror/blahajasm/bakedquad/SupportingBakedQuad";
        }
            ...
            while (iter.hasNext()) {
                ...
                if (method.name.equals("<init>") && instruction instanceof MethodInsnNode) {
                    ...
                    if (methodNode.getOpcode() == INVOKESPECIAL && methodNode.owner.equals("net/minecraft/client/renderer/block/model/BakedQuad")) {
                        methodNode.owner = "mirror/blahajasm/bakedquad/SupportingBakedQuad";
                    }
                    ...
                    if (fieldNode.owner.equals("net/minecraft/client/renderer/block/model/BakedQuad")) {
                        if (fieldsToLookOutFor.contains(fieldNode.name)) {
                            fieldNode.owner = "mirror/blahajasm/bakedquad/SupportingBakedQuad";
                        }
                    }
                }
            }
            ...
            if (method.name.equals("<init>") && method.desc.equals("(I[Ljava/lang/String;)V")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction instanceof MethodInsnNode && instruction.getOpcode() == INVOKEVIRTUAL && ((MethodInsnNode) instruction).name.equals("toLowerCase")) {
                        ...
                        iter.set(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/api/BlahajStringPool", "lowerCaseAndCanonicalize", "(Ljava/lang/String;)Ljava/lang/String;", false));
                    }
                }
            }
        }
            ...
            if (method.name.equals("<clinit>")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction instanceof TypeInsnNode && instruction.getOpcode() == NEW) {
                        ...
                        iter.add(new FieldInsnNode(GETSTATIC, "mirror/blahajasm/client/models/conditions/CanonicalConditions", bool ? "TRUE" : "FALSE", "Lnet/minecraft/client/renderer/block/model/multipart/ICondition;"));
                    }
                }
            }
        }
    ...
    private byte[] canonicalPredicatedConditions(byte[] bytes, boolean or) {
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals(getPredicate)) {
                ...
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/client/models/conditions/CanonicalConditions", or ? "orCache" : "andCache", "(Ljava/lang/Iterable;Lnet/minecraft/block/state/BlockStateContainer;)Lcom/google/common/base/Predicate;", false));
    ...
    private byte[] canonicalPropertyValueConditions(byte[] bytes) {
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals(getPredicate)) {
                ...
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/client/models/conditions/CanonicalConditions", "propertyValueCache", "(Lnet/minecraft/block/state/BlockStateContainer;Ljava/lang/String;Ljava/lang/String;Lcom/google/common/base/Splitter;)Lcom/google/common/base/Predicate;", false));
                method.instructions.add(new InsnNode(ARETURN));
    ...
    private byte[] cacheMultipartBakedModels(byte[] bytes) {
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals(makeMultipartModel)) {
                ...
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/client/models/MultipartBakedModelCache", "makeMultipartModel", "(Ljava/util/Map;)Lnet/minecraft/client/renderer/block/model/MultipartBakedModel;", false));
                method.instructions.add(new InsnNode(ARETURN));
            }
        }
                    ...
                    if (instruction instanceof MethodInsnNode) {
                        ...
                        if (methodInstruction.owner.equals("com/google/common/collect/Maps")) {
                            ...
                            if (!isExperienceList) {
                                ...
                                iter.add(new FieldInsnNode(GETSTATIC, "mirror/blahajasm/api/HashingStrategies", "FURNACE_INPUT_HASH", "Lit/unimi/dsi/fastutil/Hash$Strategy;"));
    ...
    private byte[] removePackageField(byte[] bytes) {
        ...
        try {
            ...
            packageStrings.forEach(BlahajStringPool::canonicalize);
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals("<init>") && method.desc.equals("(Ljava/io/File;Ljava/io/File;Lnet/minecraftforge/fml/common/discovery/ContainerType;ZZ)V")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD) {
                        ...
                        if (fieldNode.name.equals("packages")) {
                            ...
                            iter.previous();
                            iter.previous();
                            iter.set(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/core/BlahajHooks", "createHashSet", "()Lit/unimi/dsi/fastutil/objects/ObjectOpenHashSet;", false));
                            break;
                        }
                    }
                }
                ...
                method.instructions.add(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/core/BlahajHooks", "modCandidate$override$addClassEntry", "(Lnet/minecraftforge/fml/common/discovery/ModCandidate;Ljava/lang/String;Ljava/util/Set;Ljava/util/Set;Lnet/minecraftforge/fml/common/discovery/ASMDataTable;)V", false));
        ...
        for (MethodNode method : node.methods) {
            if (method.name.equals("<init>") && method.desc.equals("(Ljava/lang/String;)V")) {
                ...
                while (iter.hasNext()) {
                    AbstractInsnNode instruction = iter.next();
                    if (instruction.getOpcode() == PUTFIELD) {
                        method.instructions.insertBefore(instruction, new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/core/BlahajHooks", "nbtTagString$override$ctor", "(Ljava/lang/String;)Ljava/lang/String;", false));
                    }
                }
            }
        }
            ...
            if (method.name.equals("<init>") || method.name.equals("setLabel")) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD && ((FieldInsnNode) instruction).name.equals("label")) {
                        iter.previous();
                        iter.add(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/api/BlahajStringPool", "canonicalize", "(Ljava/lang/String;)Ljava/lang/String;", false));
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD) {
                        ...
                        if (fieldNode.name.equals("annotationName") || fieldNode.name.equals("className")) {
                            iter.previous();
                            iter.add(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/core/BlahajHooks", "asmData$redirect$CtorStringsToIntern", "(Ljava/lang/String;)Ljava/lang/String;", false));
                            iter.next();
                        }
                    }
                }
    ...
    private byte[] injectBlastingOilEntityItemUpdate(byte[] bytes) {
        if (!Gameplay.disableBlastingOilEvents) {
            ...
            methodVisitor.visitMethodInsn(INVOKESTATIC, "mirror/blahajasm/common/modfixes/betterwithmods/BWMBlastingOilOptimization", "inject$ItemMaterial$onEntityItemUpdate", "(Lnet/minecraft/entity/item/EntityItem;)Z", false);
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == PUTFIELD) {
                        ...
                        if (fieldNode.desc.equals("[I")) {
                            iter.previous();
                            iter.add(new VarInsnNode(ALOAD, 0));
                            iter.add(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/bakedquad/BlahajVertexDataPool", "canonicalize", "([ILnet/minecraft/client/renderer/block/model/BakedQuad;)[I", false));
                            method.maxStack = 3;
                            break;
                        }
                    }
                    ...
                    if (instruction.getOpcode() == INVOKESTATIC) {
                        ...
                        switch (methodNode.name) {
                            case "newIdentityHashMap":
                                methodNode.owner = "mirror/blahajasm/core/BlahajHooks";
                ...
                while (iter.hasNext()) {
                    ...
                    if (instruction.getOpcode() == GETFIELD) {
                        ...
                        if (getField.name.equals(renderChunkBoundingBox)) {
                            iter.set(new MethodInsnNode(INVOKESTATIC, "mirror/blahajasm/patches/RenderGlobalPatch", "getCorrectBoundingBox", "(Lnet/minecraft/client/renderer/chunk/RenderChunk;)Lnet/minecraft/util/math/AxisAlignedBB;", false));
                        }
                    }
                }
            }
