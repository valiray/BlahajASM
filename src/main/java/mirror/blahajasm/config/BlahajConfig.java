package mirror.blahajasm.config;
...
import mirror.blahajasm.config.annotation.*;
...
public class BlahajConfig {
    ...
    public void load() {
        ...
        onDemandAnimatedTextures = getBoolean("onDemandAnimatedTextures", "textures", "Calculate and send animated textures only when needed to the GPU. WARNING: Can break modded animated textures due to Pibby, if you are using Optifine you should use Smart Animations instead", false);
    ...
    public void editClassesThatCallBakedQuadCtor(Class<?> clazz) {
        ...
        if(BlahajConfig.instance.bakedQuadDynaTreeCompat) dynaTreeCompatCheck();
    }
    ...
    private void dynaTreeCompatCheck() {
        ...
        if (classes.contains("com.ferreusveritas.dynamictrees.models.bakedmodels.BakedModelBlockBranchCactus") &&
            ...
            prop.set(classes.toArray(new String[0]));
            configuration.save();
            BlahajLogger.instance.warn("BakedQuadDynaTreeCompat corrected classesThatCallBakedQuadCtor list in blahajasm.cfg");
        }
    }
