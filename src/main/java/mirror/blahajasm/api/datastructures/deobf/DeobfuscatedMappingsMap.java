...
public class DeobfuscatedMappingsMap extends Object2ObjectOpenHashMap<String, Map<String, String>> {
    ...
    public Map<String, String> put(String s, Map<String, String> innerMap) {
        if (s.indexOf('/') == -1 || s.startsWith("net/minecraft")) { // If it is a Minecraft or MinecraftForge class, we add to the map (short circuiting operation to not check innerMap strings)
            return super.put(BlahajStringPool.canonicalize(s), innerMapCanonicalCache.addOrGet(new AutoCanonizingArrayMap<>(innerMap)));
            ...
            return super.put(BlahajStringPool.canonicalize(s), innerMapCanonicalCache.addOrGet(new AutoCanonizingArrayMap<>(innerMap))); // Check if any of the values start with 'func_' or 'field_', indicating that these 99% are mappings
        }
        /*
