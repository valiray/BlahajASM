...
## 2.3.1
- Fixes `bakedQuadPatchClasses` not being properly targeted by mixins. This means, BlahajASM now requires MixinBooter. It was going to some time down the line anyways.
...
- When ChickenASM is loaded, BlahajASM fixes ChickenASM caching a shit ton of superclasses and whatnot in `ClassHierarchyManager`. This isn't used in any ASMing from what I can observe. This change is EXPERIMENTAL!
...
- Optifine now does not crash with BlahajASM. Same effect as if you disabled `bakedQuadsSquasher` in configs. The real fix will be addressed soon:tm:.
...
- Cleaned up `BlahajReflector`, potentially an API candidate.
